...
class base
{
 public:
  ...
  base(const int fd, other_args... args) : base(args...) {
    this->fd = fd;
  }

  //! @param label Name for this stopwatch.
  template< typename... other_args >
  base(const char *label, other_args... args ) : base(args...) {
    this->label = label;
  }
  ...
  //
  // @todo Do something clever with the resolution so that it doesn't report
  // more resolution than is actually available.
  //
